 ...
 * unique users per URL per date/hour
 */
public class P2Q3MRJob {

	public static void main(String[] args) throws Exception {
		if (args.length < 2) {
			System.out.println("Usage: java P2Q3MRJob <input_dir> <output_dir>");
			System.exit(-1);
		}
		// create a new MR job
		Configuration conf = new Configuration();
	    Job job = Job.getInstance(conf, "Count unique clicks per URL per hour");
	    
	    //This class is used as reference point to locate the jar file
	    job.setJarByClass(P2Q3MRJob.class);
	    
	    //set the Mapper class for the job
	    job.setMapperClass(P2Q3Mapper.class);
	    
	    //set the Reducer for the job
	    job.setReducerClass(P2Q3Reducer.class);
