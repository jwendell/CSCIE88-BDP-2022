 ...
 */
public class P2Q2MRJob {

	public static void main(String[] args) throws Exception {
		if (args.length < 2) {
			System.out.println("Usage: java P2Q2MRJob <input_dir> <output_dir>");
			System.exit(-1);
		}
		// create a new MR job
		Configuration conf = new Configuration();
	    Job job = Job.getInstance(conf, "Count unique users per URL per hour");
	    
	    //This class is used as reference point to locate the jar file
	    job.setJarByClass(P2Q2MRJob.class);
	    
	    //set the Mapper class for the job
	    job.setMapperClass(P2Q2Mapper.class);
	    
	    //set the Reducer for the job
	    job.setReducerClass(P2Q2Reducer.class);
